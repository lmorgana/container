...
namespace ft
{
	...
	public:
		...
		value_type& operator[] (const key_type& k) { return (*((this->insert(k)).first)).second; }
		...
		node_ptr search(const key_type& k) const
		{
			...
			while (!is_nil(node) && (_comp(k, *node->key) || _comp(*node->key, k)))
			{
				if (_comp(k, *node->key))
					node = node->left;
				else
	...
	private:
		...
		ft::pair<iterator, bool> insert_node(node_ptr elem, const value_type &value)
		{
			...
			if (is_nil(parent))
				_root = x;
			else if (_comp(value, *parent->key))
				parent->left = x;
			else
				parent->right = x;
